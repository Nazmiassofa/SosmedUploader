...
from services.media import FileStorage, ImageProcessor, VideoGenerator
...
class MediaService:
    ...
    def __init__(
        ...
        # Use injected R2 uploader — no duplicate instance
        self.uploader = uploader

        # Initialize sub-services
        self.processor = ImageProcessor(target_mode="auto")
        ...
        # Thread management for video generation (CPU-bound)
        self.executor = ThreadPoolExecutor(max_workers=2, thread_name_prefix="media")
    ...
    async def save_image(self, image_base64: str) -> bool:
        ...
        try:
            ...
            current_count = self.storage.get_image_count()
            if current_count >= self.max_images:
                log.warning(f"[ MEDIA ] Max images reached: {current_count}/{self.max_images}")
                images = self.storage.get_images()
                excess = current_count - self.min_images
                if excess > 0:
                    to_remove = images[:excess]
                    self.storage.cleanup_images(to_remove)
                    log.info(f"[ MEDIA ] Cleaned up {len(to_remove)} old images")

            image_bytes = self.processor.validate_base64(image_base64)
            if not image_bytes:
                return False
    ...
    def should_generate_video(self) -> bool:
        ...
        count = self.storage.get_image_count()
    ...
    def _generate_video_sync(self) -> Optional[str]:
        ...
        with self.video_generation_lock:
            ...
            try:
                ...
                # Clean up source images after successful generation
